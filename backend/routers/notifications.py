...
async def send_test_notification(current_user=Depends(get_current_user)):
    """Send a test push notification to the current user via Firebase."""
    user_id = str(current_user["_id"])
    ...
    title = "🔥 Teste de Firebase (FCM)"
    body = "Se você está vendo isso, as notificações via Firebase estão funcionando! 🎉"
    ...
    return {"message": "Notificação de teste enviada via Firebase!"}
